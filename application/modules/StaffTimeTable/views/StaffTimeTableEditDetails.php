  ...
  <div id="form" class="main col-12 h-100 py-2">
    ...
    <form class="StaffTimeTable_Insert"  id="StaffTimeTable_Insert" >
          ...
          <div class="form-group">
                 StaffUID*<select name = "StaffUID" class="form-control" id="StaffUID">
                  ...
                  foreach ($StaffField as $key => $row){ ?>
                    <?php if($StaffTimeTableField->StaffUID==$row->StaffUID){?>
                  <option value="<?php echo $row->StaffUID; ?>"selected><?php echo $row->StaffName; ?> </option>
                    <?php  } else {?>
            ...
            <div class="form-group">
                 ClassUID*<select name = "ClassUID" class="form-control" id="ClassUID">
                  ...
                  foreach ($ClassField as $key => $row){ ?>
                    <?php if($StaffTimeTableField->ClassUID==$row->ClassUID){?>
                  <option value="<?php echo $row->ClassUID; ?>"selected><?php echo $row->ClassName; ?> </option>
                    <?php  } else {?>
          ...
          <div class="form-group form-group-sm">
              ...
              value="<?php echo $StaffTimeTableField->Time; ?>" >
           ...
           <div class="form-group form-group-sm">
                ...
                <input   type="checkbox"   id="Active" name="Active" 
                 value="1" <?php $check=explode(",",$StaffTimeTableField->Active); echo in_array('1',$check)?'checked="checked"' : '' ;?>>
                <span id="Active_error" class=" Active_error text-danger"></span>
            </div>

          <div>
            <input type="hidden" name="StaffTimeTableUID" id="StaffTimeTableUID" class="StaffTimeTableUID" value="<?php echo $StaffTimeTableField->StaffTimeTableUID; ?>">
          </div>
  ...
  $(document).ready(function(){
           $('.StaffTimeTable_Insert').on('submit', function(event){
            event.preventDefault();
              $.ajax ({
                  url:"<?php echo base_url();?>StaffTimeTable/Update_StaffTimeTable_post_validate",
                  method:"POST",
                  data:$(this).serialize(),
                  dataType:"json",
